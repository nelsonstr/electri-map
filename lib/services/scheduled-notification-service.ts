...
export async function getSchedulePreferences(
  ...
  return {
    ...
    quietHoursTimezone: data.quiet_hours_timezone as string,
    dndEnabled: data.dnd_enabled as boolean,
    dndEndTime: data.dnd_end_time as string | undefined,
    preferredTimes: data.preferred_times as { label: string; time: string; daysOfWeek?: number[]; enabled: boolean }[] | undefined,
    digestEnabled: data.digest_enabled as boolean,
    digestType: data.digest_type as 'none' | 'daily' | 'weekly',
    digestTime: data.digest_time as string,
    digestDayOfWeek: data.digest_day_of_week as number | undefined,
